...
type Cmd struct {
	...
	// OS is the operating system of the process host. FUTURE: Move to cow.ProcessHost.
	OS string

	// InUVM is true if Host is a utility VM. FUTURE: Move to cow.ProcessHost.
	InUVM bool
...
func Command(host cow.ProcessHost, os string, name string, arg ...string) *Cmd {
	cmd := &Cmd{
		Host: host,
		OS:   os,
		Spec: &specs.Process{
...
func CommandContext(ctx context.Context, host cow.ProcessHost, os string, name string, arg ...string) *Cmd {
	cmd := Command(host, os, name, arg...)
	cmd.Context = ctx
	return cmd
}
