...
func TestCmdExitCode(t *testing.T) {
	cmd := Command(&localProcessHost{}, "windows", "cmd", "/c", "exit", "/b", "64")
	err := cmd.Run()
...
func TestCmdOutput(t *testing.T) {
	cmd := Command(&localProcessHost{}, "windows", "cmd", "/c", "echo", "hello")
...
func TestCmdContext(t *testing.T) {
	...
	defer cancel()
	cmd := CommandContext(ctx, &localProcessHost{}, "windows", "cmd", "/c", "pause")
	r, w := io.Pipe()
	cmd.Stdin = r
	err := cmd.Start()
	if err != nil {
...
func TestCmdStuckIo(t *testing.T) {
	cmd := Command(&stuckIoProcessHost{&localProcessHost{}}, "windows", "cmd", "/c", "echo", "hello")
